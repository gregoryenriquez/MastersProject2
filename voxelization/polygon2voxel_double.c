...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
   ...
   double CX,CY,CZ;
   int i;
   mwSize VolumeDims[3] = {0, 0, 0};
   int VolumeDimsInt[3]={0,0,0};
   ...
   VolumeDimsInt[0]=(int)VolumeSize[0]; 
   VolumeDimsInt[1]=(int)VolumeSize[1]; 
   VolumeDimsInt[2]=(int)VolumeSize[2];
   //mexPrintf("Vol: %d Vol: %d Vol: %d", VolumeDims[0], VolumeDims[1], VolumeDims[2]);
   //mexPrintf("here4.1\n");
   ...
   for (i=0; i<FacesN; i++)
   {    
        tempA = (int)FacesA[i]-1;
        tempB = (int)FacesB[i]-1;
        tempC = (int)FacesC[i]-1;
                
        Volume=draw_or_split(Volume,
                             VerticesX[tempA] - 1.0, 
                             VerticesY[tempA] - 1.0, 
                             VerticesZ[tempA] - 1.0,
                             VerticesX[tempB] - 1.0,
...
//         Volume=draw_or_split(Volume,AX,AY,AZ,BX,BY,BZ,CX,CY,CZ,VolumeDimsInt,wrap);
   }
   //mexPrintf("end\n");
}
